...
st.markdown(
    """
    <style>
    .stApp { background-color: #FFB6C1; } /* Light Pink */
...
model_path = 'models/rf_model.pkl'  # Ensure this path is correct

# Debugging: Check if file exists
if not os.path.exists(model_path):
    st.error(f"⚠️ Model file not found at `{model_path}`")
    logging.error(f"Model file not found at `{model_path}`")
    model = None  # Prevents crashes later
else:
    try:
...
with col2:
    if st.button('Detect'):
        if model is None:
            st.error("⚠️ Model not loaded. Please check the file path or reload the app.")
        else:
            try:
                ...
                if prediction[0] == 1:
                    st.markdown('<p style="color:red; font-size:18px;">🚨 You may have heart disease. Consult a doctor.</p>', unsafe_allow_html=True)
                else:
                    st.markdown('<p style="color:green; font-size:18px;">✅ You have a healthy heart! Keep up the good health.</p>', unsafe_allow_html=True)
